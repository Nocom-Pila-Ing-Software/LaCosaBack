...
def validate_card_allowed_to_trade(card, event, player):
    ...
    if card.name == "Infeccion" and player.role == "the thing":
    ...
    if card.name == "Infeccion" and amount_Infeccion_cards_in_hand == 1 and player.role == "infected":
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Player not has permission to execute this action")
    if card.name == "Infeccion" and (event.player1.role != "the thing" and event.player2.role != "the thing") and player.role == "infected":
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Player not has permission to execute this action")
    if card.name == "Infeccion" and player.role == "human":
        raise HTTPException(
